...
Key(1)=KbName('Z');
Key(2)=KbName('M');
prompt={'输入你的性别','输入你编号','职业','年龄'};
name='Basic Information';
numlines=1;
defaultanswer={'女=1，男=2','1','学生','20'};
options.Resize='on';
...
check.Slider = uicontrol(background,'style','slider','position',[cx-350 cy-240 700 39],'visible','on','Value',2.5,...
...
saveName = ['Ques_' answer{2}];
cd([mainDir,'\QuesVisualTouchPara\UserData']);
save(saveName,'answer','SectOneData','SectTwoData','SectThreeData');
